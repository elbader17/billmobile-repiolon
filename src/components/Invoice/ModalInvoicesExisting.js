...
class ModalInvoicesExisting extends React.Component {
  ...
  loadInvoice = (invoice, fiscalIdentity) => {
    Alert.alert(
      ...
      "Se cargaron todos los datos correspondientes al mismo",
      [
        {
          text: "Cargar",
          onPress: () => {
      ...
      return this.state.invoices.map((invoice) => {
        ...
        if (invoice.relationships.fiscal_identity.data != null) {
          if (!!this.state.customers && this.state.customers.length > 0) {
            fiscalIdentity = this.state.customers.find(
              (customer) =>
          ...
          >
            <View style={style.inLineSpaceBetween}>
              <View style={{ width: "35%" }}>
                <Text
                  style={
                    invoice.attributes.state === "processed"
                      ? style.textRegular12Green
                      : style.textRegular12Red
                  }
                  numberOfLines={1}
                >
      ...
      return (
        <View style={style.containerInvoicesExisting}>
          <View style={{ alignItems: "center", marginTop: 50 }}>
            ...
            />
            <Text style={style.textRegular12Blue}>Cargando Comprobantes</Text>
          </View>
        </View>
      );
    } else {
      return (
        <View style={style.containerInvoicesExisting}>
          <View style={{ flex: 0.9 }}>
            ...
            <View style={style.inLineSpaceBetween}>
              <View style={{ width: "35%" }}>
                <Text style={style.textRegular11GrayDark}> Cliente</Text>
              </View>
              <View style={{ width: "12%" }}>
                <Text style={style.textRegular11GrayDark}>Tipo</Text>
              </View>
              <View style={{ width: "13%" }}>
                <Text style={style.textRegular11GrayDark}>Fecha</Text>
              </View>
              <View style={{ width: "20%" }}>
                <Text style={style.textRegular11GrayDark}>Total</Text>
              </View>
              <View style={{ width: "10%" }}>
                <Text style={style.textRegular11GrayDark}> </Text>
              </View>
              <View style={{ width: "10%" }}>
                <Text style={style.textRegular11GrayDark}> </Text>
              </View>
            ...
            <View style={style.listInvoices}>
              <ScrollView>{this.renderListInvoices()}</ScrollView>
            </View>
