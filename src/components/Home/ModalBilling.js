  ...
  renderListInvoices = (month) => {
    const filterInvoices = this.props.invoices.filter(
      (invoice) =>
        ...
        invoice.attributes.invoice_date.slice(5, 7) === month
    ); //Filtramos facturas procesadas y por mes
    if (filterInvoices.length === 0) {
      ...
      return filterInvoices
        .sort(function (a, b) {
          //Ordenamos por dia
          if (
            a.attributes.invoice_date.slice(8, 10) >
            b.attributes.invoice_date.slice(8, 10)
          ) {
            return 1;
          }
          if (
